...
(define (test-empty-and-add-node)
  (let ((n (empty-net)))
    ...
    ;; adding the same node with V -> A upgrades
    (add-node! n 'bar 'V)
...
(define (test-rewire_and_delete)
  (let ((n (empty-net)))
    ...
    ;; delete z and ensure links removed
    (delete-node! n 'z)
...
(define (test-pretty-print_and_options)
  (let ((n (empty-net)))
    ...
    ;; show-V? default false -> v1 omitted
...
(define (test-make-fresh-name)
  (let ((n (empty-net)))
    ...
    ;; produce many names to exercise counter progression
    (let loop ((i 0) (acc #t))
      (if (>= i 200)
          acc
...
(define (test-link-conflict-modes)
  (let ((n (empty-net)))
    ...
    (add-node! n 'b 'A)
    (add-node! n 'c 'A)
    ;; default mode parameter should be 'error
    (assert-eq (*link-conflict-mode*) 'error "default conflict mode is error")
    ;; set to overwrite-injection and ensure conflicting link overwrites
    (set-link-conflict-mode! 'overwrite-injection)
    (link-peers! n (endpoint 'a 'p) (endpoint 'b 'p))
    ;; create conflict: link a.p -> c.p (should overwrite existing mapping)
    (link-peers! n (endpoint 'a 'p) (endpoint 'c 'p))
    (assert-eq (peer n (endpoint 'a 'p)) (endpoint 'c 'p) "overwrite-injection replaced peer")
    ;; set to warn: should not throw
    (set-link-conflict-mode! 'warn)
    (link-peers! n (endpoint 'a 'p) (endpoint 'b 'p)) ;; existing conflict, should warn but not error
    ;; restore to error to avoid side-effects for other tests
...
(define (test-parse-net_and_parse-endpoint)
  (let* ((sexpr '(par (node A A) (node B A) (wire (A p) (B p))))
    ...
    ;; endpoints provided as (list name port)
...
(define (test-unlink-by-equal-key)
  ;; ensure unlink_port! removes links even when the passed cons is not eq? to the stored key
  (let ((n (empty-net)))
    (parse-net '(par (node a A) (node b A) (wire (a p) (b p))))
    ;; create a fresh cons with same values and unlink it
...
(define (test-validate_copy_isolation)
  (let ((n (parse-net '(par (node u A) (node v A) (wire (u p) (v p))))))
    (let ((c (copy-net n)))
      ...
      (delete-node! n 'v)
      ;; original should no longer contain v
      (assert-false (node-agent n 'v) "original lost v after delete")
      ;; copy should still contain v and its node present
      (assert-true (node-agent c 'v) "copied net still contains v")
      ;; verify the copy's links include an entry for u.p (scan links to avoid eq? hashing issues)
      (let ((found #f))
