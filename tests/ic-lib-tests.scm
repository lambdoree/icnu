...
(define (test-IC_PURE_PAIR_FST_SND)
  (let* ((v1 10) (v2 "v2")
         (pair-sexpr `(par ,(IC_LITERAL v1 'in1)
                           ,(IC_LITERAL v2 'in2)
                           ,(IC_PURE_PAIR 'in1 'in2 'p-out))))
    ;; We can't directly eval a pair, but we can check its components by applying FST and SND.
    (let* ((fst-sexpr `(par ,@(cdr pair-sexpr)
                           ,(IC_PURE_FST '(p-out p) 'fst-out)))
           (snd-sexpr `(par ,@(cdr pair-sexpr)
...
(define (test-IC_PURE_constructors_are_valid_nets)
  "Tests that macros for LEFT, RIGHT, and EITHER expand to parsable net expressions."
