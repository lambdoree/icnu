...
(define (escape-ml-label s)
 ...
 (let* ((raw (if (symbol? s) (symbol->string s) (format-string #f "~a" s)))
  (escaped-backslash (icnu-string-replace raw "\\" "\\\\"))
 (escaped-backslash ewqewqeq)
  (escaped-quote (icnu-string-replace escaped-backslash "\"" "\\\""))
  (escaped-newline (icnu-string-replace escaped-quote "\n" "\\n"))
  (escaped-lb (icnu-string-replace escaped-newline "[" "_"))
  (escaped-rb (icnu-string-replace escaped-lb "]" "_"))
  (escaped-lp (icnu-string-replace escaped-rb "(" "_"))
  (escaped-rp (icnu-string-replace escaped-lp ")" "_"))
  (escaped-lc (icnu-string-replace escaped-rp "{" "_"))
  (escaped-rc (icnu-string-replace escaped-lc "}" "_")))
  escaped-rc))
...
and avoids using dynamic format calls that may receive unexpected types."
 ...
 (hash-for-each
   (lambda (name agent)
   (when (not (eq? agent 'V))               ; hide V‑nodes (free names)
   ...
   (if (string? raw) raw (format-string #f "~a" raw))))
    ...
    (color (cond ((eq? agent 'A) "#f9f")
          ...
          ((eq? agent 'E) "#ff9")
          (else "#fff")))
    (shape (cond ((eq? agent 'A) (format-string #f "~a[~a]" id lbl-esc))   ; rectangle
          ((eq? agent 'C) (format-string #f "~a(~a)" id lbl-esc))   ; circle
          ((eq? agent 'E) (format-string #f "~a{~a}" id lbl-esc))   ; rhombus
          (else (format-string #f "~a[~a]" id lbl-esc)))))          ; fallback
    ;; Group the node shape and its port nodes inside a single subgraph so that
    ;; Mermaid renders the agent label and the ports visually grouped together.
