    ...
    (cond
     ((= len 4)
      (let ((a (unquote-if-needed (list-ref args 0)))
        ...
        (unless (and (symbol? a) (valid-port? p) (symbol? b) (valid-port? q))
     ...
     ((= len 3)
      (if (list? (list-ref args 2))
          (let ((a (unquote-if-needed (list-ref args 0)))
                (p (unquote-if-needed (list-ref args 1)))
                (ep2 (list-ref args 2)))
            (unless (and (symbol? a) (valid-port? p)) (error "parse-wire-args: Bad 3-argument wire form. Expected (wire 'a 'p ('b 'q))." args))
          ...
          (let ((ep1 (list-ref args 0))
            ...
            (unless (and (list? ep1) (symbol? b) (valid-port? q))
